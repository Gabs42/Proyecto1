...
using namespace std;
typedef class memoria{//estructura de almacenamiento de memoria
public:
    char key[6]="vacio";
    void* memoria_real;//almacenamiento de dato
    bool activo=true;
    bool en_uso;
    memoria *memoriaSiguiente =NULL;
    int size;
    int espacioMemoria;//espacio en memoria del bloque
    int operacion =1000;//marca el tipo de operacion(get delete etc)
    bool operator==(memoria mem){//operadores modificados
...
ptrmemoria puntero;//puntero de lista de memoria nrmal
ptrmemoria punteroCache;//puntero de lista de memoria cache
...
int main(){
    ...
    struct sockaddr_in server_aadr;//address del socket
    socklen_t size;
    client = socket(AF_INET, SOCK_STREAM,0);
    if(client<0){//revisa si el cliente es  valido
    ...
    server_aadr.sin_port = htons(portNum);//intenta conectarse a el puerto indicado
    ...
    while(endServer==false){
        listen(client,1);//espera un mensaje del cliente
...
void inicializarMemoria(){//inicia la lista de la memoria
    int y=40;
    int flag=1;
    puntero = new(memoria);
...
void incializarCache(){//inicia la cache
...
void actualizarCache(ptrmemoria memoria){//mueve la cache para que ell mas reciente este al frente
    char comp[6] = "vacio";
    ptrmemoria memActual = punteroCache;
...
ptrmemoria rm_get(char* key){//devuelve la memoria con la key indicada
...
void rm_delete(ptrmemoria mem){//borra la memoria indicada
    ptrmemoria memCache = punteroCache;
    ptrmemoria memCachepasada = punteroCache;
    while(memCache != NULL){
...
void ordenarCache(ptrmemoria memoriaMover,ptrmemoria memoriaPasada){//swap de la cache
...
void insertarEnMemoria(ptrmemoria memoria){//insera la nueva memoria en la lista
    char comp[6] = "vacio";
    ptrmemoria memActual = puntero;
...
void limpiarMemoria(){//limpia la cache y la memoria principal cambio lo activo a inactivo y borra lo inactivo
    ptrmemoria memCache = punteroCache;
    ptrmemoria memCachepasada = punteroCache;
    while(memCache != NULL){
...
void verMemoria(){//imprime el estado de la memoria
    char comp[6] = "vacio";
